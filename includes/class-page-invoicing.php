...
class MyNJILGA_Page_Invoicing {
    ...
    private static function row_actions( object $row, array $c, bool $canCreate ): string {
        ...
        switch ( $c['status'] ) {
            ...
            case 'created':
                return sprintf(
                    '<button type="button" class="njilga-btn njilga-btn-primary njilga-btn-sm" data-send="%d">Send</button> %s %s %s %s',
                    $rowId,
                    self::view_link( $link ),
                    self::pdf_link( $pdf ),
            ...
            case 'sent':
                return sprintf(
                    '%s %s <button type="button" class="njilga-btn njilga-btn-ghost njilga-btn-sm" data-send="%d">Resend</button> %s %s',
                    self::view_link( $link ),
                    self::pdf_link( $pdf ),
                    $rowId,
                    self::void_button( $row ),
            ...
            case 'processing':
                return sprintf(
                    '<span class="njilga-dim njilga-inline-status">%s Processing (ACH)</span> %s %s %s %s',
                    self::icon( 'refresh' ),
                    self::view_link( $link ),
                    self::pdf_link( $pdf ),
                    self::void_button( $row ),
